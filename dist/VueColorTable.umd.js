(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["VueColorTable"] = factory();
	else
		root["VueColorTable"] = factory();
...
function rgbStringToArr(str) {
  var arr = str.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+(?:\.\d+)?))?\)$/).splice(1);
...
function rgbaArrToString(arr) {
  return "rgba(".concat(arr[0], ", ").concat(arr[1], ", ").concat(arr[2], ", ").concat(arr[3], ")");
}
...
/* harmony default export */ var utils = ({
  isHex: isHex,
  isRgb: isRgb,
  rgbStringToArr: rgbStringToArr,
  rgbaArrToString: rgbaArrToString,
  rgbToHex: rgbToHex,
  hexToRgb: hexToRgb,
  hexToRgba: hexToRgba,
  gradient: gradient,
  getIndexByObjName: getIndexByObjName,
  linearMap: linearMap
});
...
ZbbPlot.prototype = {
  // 画字体
  drawText: function drawText(opts) {
    var maginLeft = 0;
    var maginTop = 6;
    var style = opts.style || {};
    this.ctx.beginPath();

    for (var key in DEFAULT_STYLE_CONFIG.TEXT) {
      this.ctx[key] = DEFAULT_STYLE_CONFIG.TEXT[key];
    }

    for (var _key in style) {
      this.ctx[_key] = style[_key];
    }
  ...
  // 画线
  drawLine: function drawLine(opts) {
    var style = opts.style || {};
    this.ctx.beginPath();

    for (var key in DEFAULT_STYLE_CONFIG.LINE) {
      this.ctx[key] = DEFAULT_STYLE_CONFIG.LINE[key];
    }

    for (var _key2 in style) {
      this.ctx[_key2] = style[_key2];
    }
  ...
  createLinearGradient: function createLinearGradient(x, y, x2, y2) {
    return this.ctx.createLinearGradient(x, y, x2, y2);
  },
  // 线条像素优化
  subPixel: function subPixel(lineWidth) {
    if (lineWidth % 2 == 1) {
      this.halfPixel = 0.5;
    } else {
      this.halfPixel = 0;
  ...
  addClick: function addClick(x, y, dragging) {
    this.clickX.push(x);
    this.clickY.push(y);
    this.clickDrag.push(dragging);
  },
  reDrawLineByMouse: function reDrawLineByMouse(opts) {
    var option = opts || {};
    var style = option.style || {};

    for (var key in DEFAULT_STYLE_CONFIG.LINE_BY_MOUSE) {
      this.ctx[key] = DEFAULT_STYLE_CONFIG.LINE_BY_MOUSE[key];
    }

    for (var _key3 in style) {
      this.ctx[_key3] = style[_key3];
    }
...
/* harmony default export */ var zbbPlot = (ZbbPlot);
// CONCATENATED MODULE: ./src/js/colorBar.js
...
function ColorBar(dom, opts) {
  ...
  this.scaleCount = 100;
  this.axisTextInterval = 100; // 刻度盒子
...
ColorBar.prototype = {
  ...
  drawBar: function drawBar() {
    var grd = this.zp.createLinearGradient(this.barBoundary.x, this.barBoundary.y, this.barBoundary.x, this.barBoundary.y + this.barBoundary.height);
  ...
  // 画字体
  drawAxisText: function drawAxisText() {
    var _this$opts$minMax = _slicedToArray(this.opts.minMax, 2),
        min = _this$opts$minMax[0],
        max = _this$opts$minMax[1];

    var range = max - min;
    var textCount = range / this.axisTextInterval;
    var _ref = [this.axisTextBoundary.x, this.axisTextBoundary.y],
        orginX = _ref[0],
        orginY = _ref[1];
  ...
  // 画坐标刻度
  drawAxisScale: function drawAxisScale() {
    var _this$opts$minMax2 = _slicedToArray(this.opts.minMax, 2),
        min = _this$opts$minMax2[0],
        max = _this$opts$minMax2[1];

    var range = max - min;
    var textCount = range / this.axisTextInterval;
    var _ref2 = [this.barBoundary.x, this.barBoundary.y],
        orginX = _ref2[0],
        orginY = _ref2[1];
    var shortTick = 3;
    var longTick = 6;
    var tickLength;
    var scaleCount = this.scaleCount;
...
/* harmony default export */ var colorBar = (ColorBar);
// CONCATENATED MODULE: ./src/js/opacityChart.js
...
OpacityChart.prototype = {
  ...
  drawGrid: function drawGrid() {
    ...
    for (var i = 0; i < this.opts.data.length - 1; i++) {
      var step = parseInt((this.opts.data[i + 1].value - this.opts.data[i].value) * 100);
      var colorList = this.zp.utils.gradient(this.opts.data[i].color, this.opts.data[i + 1].color, step);

      for (var j = 0; j < step; j++) {
        var grd = this.zp.createLinearGradient(this.gridBoundary.x, this.gridBoundary.y, this.gridBoundary.x, this.gridBoundary.y + this.gridBoundary.height);
  ...
  initDrawOpacityLine: function initDrawOpacityLine() {
    this.opacityList = [];
    this.opacityPointY = [];
    this.colorPostion = [];
    ...
    for (var _i = 0; _i < this.opts.data.length - 1; _i++) {
      var step = parseInt(((this.opts.data[_i + 1].value - this.opts.data[_i].value) * 100).toFixed(0));

      for (var j = 0; j < step; j++) {
        if (j == 0) {
          this.colorPostion.push(this.opacityPointY.length);
          this.opacityPointY.push(this.opacityList[_i]);
        } else {
  ...
  drawOpacityLine: function drawOpacityLine() {
    for (var i = 0; i < this.opacityPointY.length; i++) {
      var pointX = this.dataToPoint(i, [0, 100], [this.gridBoundary.x + 2, this.gridBoundary.x + 2 + this.gridBoundary.width]);
      var pointY = this.dataToPoint(this.opacityPointY[i], [1, 0], [this.gridBoundary.y, this.gridBoundary.y + this.gridBoundary.height]);
  ...
  drawAxisScale: function drawAxisScale() {
    ...
    var range = max - min;
    var textCount = range / this.axisTextInterval;
    var _ref2 = [this.gridBoundary.x, this.gridBoundary.y],
        orginX = _ref2[0],
        orginY = _ref2[1];
    var shortTick = 3;
    var longTick = 6;
  ...
  // 像素坐标转数据
  pointToData: function pointToData(point, domain, range) {
...
/* harmony default export */ var opacityChart = (OpacityChart);
...
/* harmony default export */ var lib_vue_loader_options_srcvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    setBgStyleByColor: function setBgStyleByColor(index) {
      return {
        width: '100%',
        height: '25px',
    ...
    setBgStyleByColorArr: function setBgStyleByColorArr(index) {
      return {
        width: '100%',
        height: '25px',
    ...
    parseTableData: function parseTableData(obj) {
      var arr = [];
    ...
    parseBgcolorTemplate: function parseBgcolorTemplate(val) {
      var str = '';
      val.colors.forEach(function (item) {
        str += ',' + item[1] + ' ' + item[0] * 100 + '%';
      });
      return "linear-gradient(to right ".concat(str, ")");
    },
    setLutTableActive: function setLutTableActive(index) {
    ...
    parseLutItem: function parseLutItem(colors) {
      var _this = this;
    ...
    addLutItem: function addLutItem(items) {
    ...
    deleteLutItem: function deleteLutItem(index) {
    ...
    modifyLutItem: function modifyLutItem(index, items) {
      var colors = [];
    ...
    deleteColorItem: function deleteColorItem(indexArr) {
      for (var i = indexArr.length - 1; i >= 0; i--) {
        this.lutItem.splice(indexArr[i], 1);
      }
    },
    addColorItem: function addColorItem(newList) {
    ...
    handleTrigger: function handleTrigger() {
      ...
      this.$nextTick(function () {
        _this2.setLutTableActive(_this2.currentIndex);

        _this2.initColorBar();

        _this2.initOpacityChart();
      });
    },
    ...
    parseOpacityToLutItem: function parseOpacityToLutItem() {
      for (var i = 0; i < this.lutItem.length; i++) {
        var splitArr = this.lutItem[i].color.split(',');
        splitArr[3] = "".concat(this.oc.opacityList[i], ")");
        this.lutItem[i].color = splitArr.join(',');
      }

      this.handleColorPickerChange();
    },
    handleBtnSave: function handleBtnSave() {
      if (!this.currentName) {
        return this.$message({
          showClose: true,
          message: '名称为空！',
    ...
    handleSelectionChange: function handleSelectionChange(selections) {
      ...
      selections.forEach(function (val) {
        _this3.lutItem.forEach(function (v, i) {
    ...
    handleBtnInterpolation: function handleBtnInterpolation() {
      ...
      var newList = [];
      var newCount = parseInt(this.ipNum);
      var step = newCount + 1;
      var startValue = this.lutItem[this.selectIndex[0]].value;
      var endValue = this.lutItem[this.selectIndex[1]].value;
      var range = endValue - startValue;
      var newColorList = utils.gradient(this.lutItem[this.selectIndex[0]].color, this.lutItem[this.selectIndex[1]].color, step);
    ...
    handleBtnRemove: function handleBtnRemove() {
    ...
    handleColorPickerChange: function handleColorPickerChange() {
      this.refreshColorBar();
      this.refreshOpacityChart();
    },
    initColorBar: function initColorBar() {
      var opts = {
        data: this.lutItem,
        minMax: this.minMax
      };
    ...
    refreshColorBar: function refreshColorBar() {
      var opts = {
        data: this.lutItem,
        minMax: this.minMax
      };
    ...
    refreshOpacityChart: function refreshOpacityChart() {
      var opts = {
        data: this.lutItem,
        minMax: [0.0, 1.0]
      };
